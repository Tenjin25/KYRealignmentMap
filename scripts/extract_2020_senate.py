"""
Extract 2020 US Senate Election Results from Kentucky Secretary of State PDF.
Mitch McConnell (REP) vs Amy McGrath (DEM) vs Brad Barron (LIB) + 5 write-ins.
"""
...
CANDIDATES_2020_SENATE = [
    {'name': 'Mitch McConnell', 'party': 'REP'},
    {'name': 'Amy McGrath', 'party': 'DEM'},
    {'name': 'Brad Barron', 'party': 'LIB'},
    {'name': 'Paul Frangedakis', 'party': 'WI'},
    {'name': 'John Daniel Cobble', 'party': 'WI'},
    {'name': 'Randall Lee Teegarden', 'party': 'WI'},
    {'name': 'Harold H. Fitzpatrick', 'party': 'WI'},
    {'name': 'Demetra Wysinger', 'party': 'WI'}
]
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page in pdf.pages[6:12]:
            ...
            for line in lines:
                ...
                for i, cand_info in enumerate(CANDIDATES_2020_SENATE):
                    if i < len(votes):
                        results.append({
                            'county': county,
                            'office': 'U.S. Senate',
...
def main():
    pdf_path = "data/2020 General Election Results.pdf"
    ...
    print("="*70)
    print("Extracting 2020 US Senate Election (Kentucky)")
    print("="*70)
    print(f"\nPDF: {Path(pdf_path).name}")
    print(f"Race: Mitch McConnell (REP) vs Amy McGrath (DEM)\n")
    ...
    output_file = Path('data') / '20201103__ky__general__senate__county.csv'
